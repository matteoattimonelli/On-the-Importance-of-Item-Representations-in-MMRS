...
def main():
    ...
    if not os.path.exists(logs_path + f'/{args.data}/mean_neigh/{args.model}/') and args.cluster:
        os.makedirs(logs_path + f'/{args.data}/mean_neigh/{args.model}/')

    if not os.path.exists(scripts_path + f'/{args.data}/mean_neigh/{args.model}/') and args.cluster:
        os.makedirs(scripts_path + f'/{args.data}/mean_neigh/{args.model}/')
    ...
    for hyperparam in hyperparams:
        ...
        if args.cluster == 'cineca':
            command_line = (f'CUBLAS_WORKSPACE_CONFIG=:4096:8 python {script} {to_cmd(hyperparam)} '
                            f'--data={args.data} '
                            f'--method=neigh_mean '
                            f'--model={args.model} > {logs_path}/{args.data}/mean_neigh/{args.model}/{logfile} 2>&1')
...
#SBATCH --output=/leonardo/home/userexternal/dmalites/slogs/SisInf_Missing_Multimod_output-%A_%a.out
#SBATCH --error=/leonardo/home/userexternal/dmalites/slogs/SisInf_Missing_Multimod_error-%A_%a.err
#SBATCH --account={1}
...
"""
    ...
    if header:
        for index, offset in enumerate(range(0, nb_jobs, args.batch_size_jobs), 1):
            ...
            with open(scripts_path + f'/{args.data}/mean_neigh/{args.model}/' + date_time + f'__{index}.sh', 'w') as f:
                if args.cluster == 'cineca':
        ...
        with open(f'run_multimodal_all_neigh_mean_{args.data}_{args.model}.sh', 'w') as f:
