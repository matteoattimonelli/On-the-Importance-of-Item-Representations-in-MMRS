...
def main():
    ...
    if not os.path.exists(logs_path + f'/{args.dataset}/gae/{args.model}/') and args.cluster:
        os.makedirs(logs_path + f'/{args.dataset}/gae/{args.model}/')

    if not os.path.exists(scripts_path + f'/{args.dataset}/gae/{args.model}/') and args.cluster:
        os.makedirs(scripts_path + f'/{args.dataset}/gae/{args.model}/')
    ...
    for hyperparam in hyperparams:
        ...
        if args.cluster == 'cineca':
            command_line = (f'CUBLAS_WORKSPACE_CONFIG=:4096:8 python {script} {to_cmd(hyperparam)} '
                            f'--data={args.dataset} '
                            f'--method=gae '
                            f'--model={args.model} > {logs_path}/{args.dataset}/gae/{args.model}/{logfile} 2>&1')
...
"""
    ...
    if header:
        for index, offset in enumerate(range(0, nb_jobs, args.batch_size_jobs), 1):
            ...
            with open(scripts_path + f'/{args.dataset}/gae/{args.model}/' + date_time + f'__{index}.sh', 'w') as f:
                if args.cluster == 'cineca':
        ...
        with open(f'run_multimodal_all_gae_{args.dataset}_{args.model}.sh', 'w') as f:
