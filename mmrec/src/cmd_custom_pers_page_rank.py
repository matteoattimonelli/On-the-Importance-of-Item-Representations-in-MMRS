...
def main():
    ...
    if not os.path.exists(scripts_path + f'/{args.data}/pers_page_rank/{args.model}/'):
        os.makedirs(scripts_path + f'/{args.data}/pers_page_rank/{args.model}/')
    ...
    for hyperparam in hyperparams:
        ...
        if args.cluster == 'cineca':
            command_line = (f'CUBLAS_WORKSPACE_CONFIG=:4096:8 python {script} {to_cmd(hyperparam)} '
                            ...
                            f'--model={args.model} > {logs_path}/{args.data}/pers_page_rank/{args.model}/{logfile} 2>&1')
...
"""
    ...
    if header:
        for index, offset in enumerate(range(0, nb_jobs, args.batch_size_jobs), 1):
            ...
            with open(scripts_path + f'/{args.data}/pers_page_rank/{args.model}/' + date_time + f'__{index}.sh', 'w') as f:
                if args.cluster == 'cineca':
