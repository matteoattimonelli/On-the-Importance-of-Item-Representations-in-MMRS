...
class Similarity(object):
    ...
    def __init__(self, data, attribute_matrix, num_neighbors, similarity, implicit, **kwargs):
        self._data = data
    ...
    def initialize(self):
        ...
        if self._similarity == "cosine":
             W_sparse = sim.cosine(self._attribute_matrix, k=self._num_neighbors, format_output='csr')
        elif self._similarity == "asym":
             W_sparse = sim.asymmetric_cosine(self._attribute_matrix, alpha=self._alpha, k=self._num_neighbors, format_output='csr')
        elif self._similarity == "dot":
             W_sparse = sim.dot_product(self._attribute_matrix, k=self._num_neighbors, format_output='csr')
        elif self._similarity == "jaccard":
             W_sparse = sim.jaccard(self._attribute_matrix, k=self._num_neighbors, binary=True, format_output='csr')
        elif self._similarity == "dice":
             W_sparse = sim.dice(self._attribute_matrix, k=self._num_neighbors, binary=True, format_output='csr')
        elif self._similarity == "tversky":
             W_sparse = sim.tversky(self._attribute_matrix, k=self._num_neighbors, alpha=self._tversky_alpha, beta=self._tversky_beta, binary=True, format_output='csr')
        # self._item_ratings = {}
